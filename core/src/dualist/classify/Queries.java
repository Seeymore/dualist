...
public class Queries {
	...
	public static Multimap<Integer, String> randomFeaturesPerLabel(HashMultimap<Integer, String> labeledFeatures, InstanceList ilist, int num) {
	...
	public static Multimap<Integer, String> commonFeaturesPerLabel(HashMultimap<Integer, String> labeledFeatures, InstanceList ilist, int num) {
	...
	public static InstanceList queryByCommittee(List<Classifier> qbcModels, InstanceList poolData, int num, String mode) {

		List<HashMap<Instance, Double>> instanceMapList = new ArrayList<HashMap<Instance, Double>>();

		for (Classifier c : qbcModels) {
			HashMap<Instance, Double> instanceMap = Maps.newHashMap();
			for (Instance instance : poolData) {
				LabelVector lv = c.classify(instance).getLabelVector();
				// entropy query strategy
		...
		List<Entry<Instance, Double>> rankedEntries;

		if (instanceMapList.size() == 1) {
			rankedEntries = rankMapByValue(instanceMapList.get(0));
		} else {
			Map<Instance, Double> confidenceDifferenceMap = Maps.newHashMap();

			Iterator<HashMap<Instance, Double>> it = instanceMapList.iterator();
			Map<Instance, Double> firstMap = it.next();
			while (it.hasNext()) {
				Map<Instance, Double> otherMap = it.next();
				for (Map.Entry<Instance, Double> entry : firstMap.entrySet()) {
					Instance instance = entry.getKey();
					Double confidenceA = entry.getValue();
					Double confidenceB = otherMap.get(instance);
					confidenceDifferenceMap.put(instance, Math.abs(confidenceA - confidenceB));
				}
			}

			rankedEntries = rankMapByValue(confidenceDifferenceMap);
		}

		System.err.println(rankedEntries.toString());
		InstanceList ret = poolData.cloneEmpty();
		int numQueries = 0;
		for (Map.Entry<Instance, Double> entry : rankedEntries) {
			Instance instance = entry.getKey();
			ret.add(instance);
			numQueries++;
			if (numQueries >= num)
	...
	public static double entropy(double[] probs) {
		...
		for (int i = 0; i < probs.length; i++)
			ret -= (probs[i] > 1e-7) ? probs[i] * Math.log(probs[i]) : 0;
		return ret;
	}
