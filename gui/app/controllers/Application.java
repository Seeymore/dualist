...
public class Application extends Controller {
	...
	public static void setupExperiment(@Required String username, @Required File trainFile, File testFile, @Required String type, @Required String mode, @Required int numMinutes,
			@Required int numInstances) throws Exception {
	...
	public static void learn(String features, String instances, String log) throws Exception {
		...
		for (String labeledInstance : instances.trim().split("\\s+")) {
			if (!labeledInstance.isEmpty()) {
				...
				// slow, but offhand the best way to search for a labeled
				// instance...
		...
		String modelFile = MODELS_DIR + getTrialID() + ".NaiveBayes." + String.format("%04d", (int) timeSoFar) + ".model";
		...
		boolean instancesCovered = (labelAlphabet.size() == instLabels.size()) && (labeledSet.size() > 1.5 * labelAlphabet.size());
		...
		// if we're in passive mode, or have insufficient labels, do passive
		// selection
		if (mode.equals("passive") || (!featuresCovered && !instancesCovered)) {
			...
			// queryFeatures = Queries.randomFeaturesPerLabel(labeledFeatures,
			// unlabeledSet, 50);
			queryFeatures = Queries.commonFeaturesPerLabel(labeledFeatures, unlabeledSet, NUM_FEATURE_QUERIES);
		}

		// if mode is qbc, do query by committee
		else if (mode.equals("qbc")) {
			// set up additional trainer objects
			List<ClassifierTrainer> qbcTrainers = new ArrayList<ClassifierTrainer>();
			List<Classifier> qbcModels = new ArrayList<Classifier>(Arrays.asList(nbModel));
			qbcTrainers.add(new MaxEntTrainer());
			qbcTrainers.add(new C45Trainer());

			// train and store model in list
			for (ClassifierTrainer trainer : qbcTrainers) {
				try {
					Classifier model = trainer.train(labeledSet);
					qbcModels.add(model);

				} catch (OptimizationException oe) {
					Logger.error("Error during optimization of classifier: " + trainer.getClass().getName());
				}
			}

			Logger.info("QUERY BY COMMITTEE");
			logResult(timeSoFar + "\tQBC");

			queryInstances = Queries.queryByCommittee(qbcModels, unlabeledSet, numInstances, "entropy");
			// do the per-label query thang
			queryFeatures = Queries.queryFeaturesPerLabelMI(nbModel, labeledFeatures, Util.probabilisticData(nbModel, labeledSet, unlabeledSet), NUM_FEATURE_QUERIES);
		}
	...
	private static void logResult(String string) {
		try {
			Files.append(string.trim() + "\n", new File(RESULTS_DIR + getTrialID() + ".txt"), Charsets.UTF_8);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public static void predict(String features, String instances) {
		...
		for (Instance instance : unlabeledSet) {
			...
			sb.append(l.getBestLabel().toString() + "\t" + df.format(l.getBestValue()) + "\t" + instance.getName() + "\t" + summary + "\n");
		}
		...
		for (Instance instance : labeledSet) {
			...
			if (summary.length() > 150)
				summary = summary.substring(0, 150) + "...";
			sb.append("# " + l.getBestLabel().toString() + "\t" + instance.getName() + "\t" + summary + "\n");
		}
